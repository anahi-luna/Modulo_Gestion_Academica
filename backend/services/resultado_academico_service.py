...
from clients.planes_cliente import (
    obtener_plan_asignatura_por_comision_asignatura,
    obtener_id_plan_por_comision_asignatura,
)
...
def obtener_inscripciones_aceptadas(id_comision_asignatura):

    return Inscripcion.query.filter_by(
        id_comision_asignatura=id_comision_asignatura, id_estado=2  # Aceptada
    ).all()
...
def obtener_clases_dictadas(id_comision_asignatura):

    return (
        Clase.query.filter_by(
            id_comision_asignatura=id_comision_asignatura, estado=EstadoClase.DICTADA
        )
...
def finalizar_inscripcion(inscripcion):
    ...
    estado = obtener_estado_por_nombre("Finalizada")
...
def calcular_porcentaje_asistencia(inscripcion):
    ...
    clases_dictadas = obtener_clases_dictadas(inscripcion.id_comision_asignatura)
...
def validar_comision_finalizada(id_comision_asignatura):

    # Busca clases pendientes de dictado.
    clases_pendientes = Clase.query.filter(
        Clase.id_comision_asignatura == id_comision_asignatura,
        Clase.estado.in_([EstadoClase.PROGRAMADA, EstadoClase.REPROGRAMADA]),
    ).count()

    if clases_pendientes > 0:

        raise BusinessError("La comisión asignatura todavía no finalizó.", 400)
...
def determinar_estado_academico(porcentaje_asistencia, promedio_final, reglas):
    ...
    if porcentaje_asistencia < reglas["presentismo_porc"]:

        return obtener_estado_academico_por_nombre("Libre")

    if promedio_final >= reglas["final_aprobacion"]:

        return obtener_estado_academico_por_nombre("Aprobado")

    if promedio_final >= reglas["regularizacion_prom"]:
...
def crear_resultado_academico(datos):
    ...
    try:
        ...
        inscripciones = obtener_inscripciones_aceptadas(datos["id_comision_asignatura"])

        if not inscripciones:

            raise BusinessError("La comisión asignatura no posee alumnos aceptados.", 400)
        ...
        for resultado in resultados:
            ...
            actualizar_resultado_plan(inscripcion.id_legajo,id_plan)
