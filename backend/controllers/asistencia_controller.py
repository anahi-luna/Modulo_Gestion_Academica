...
def get_lista_de_asistencias():
    ...
    id_clase = request.args.get("id_clase", type=int)

    id_inscripcion = request.args.get("id_inscripcion", type=int)

    asistencias = obtener_lista_de_asistencias(
        id_clase=id_clase, id_inscripcion=id_inscripcion
    )

    resultado = asistencias_schema.dump(asistencias)

    return success_response(
        data=resultado, total=len(resultado), message="Listado de asistencias."
    )
...
def get_asistencia(id_asistencia):
    ...
    if not asistencia:
        ...
        return error_response("Asistencia no encontrada.", status_code=404)
...
def agregar_asistencias():
    ...
    try:
        ...
        datos = asistencia_request_schema.load(request.get_json())

        asistencias = crear_asistencias(datos)
...
def actualizar_asistencia(id_asistencia):
    ...
    try:
        ...
        datos = modificar_asistencia_schema.load(request.get_json())

        asistencia = modificar_asistencia(id_asistencia, datos)
        ...
        return success_response(
            data=resultado, message=f"Asistencia {id_asistencia} actualizada."
        )
...
def eliminar_asistencia_controller(id_asistencia):

    try:

        eliminado = eliminar_asistencia(id_asistencia)

        if not eliminado:

            return error_response("Asistencia no encontrada.", status_code=404)

        return success_response(message=f"Asistencia {id_asistencia} eliminada.")
