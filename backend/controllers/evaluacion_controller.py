...
def get_lista_de_evaluaciones():
    ...
    id_comision = request.args.get(
        "id_comision",
        type=int
    )

    id_tipo_evaluacion = request.args.get(
        "id_tipo_evaluacion",
        type=int
    )
    ...
    resultado = evaluaciones_schema.dump(
        evaluaciones
    )
...
def get_evaluacion(id_evaluacion):
    ...
    resultado = evaluacion_schema.dump(
        evaluacion
    )
...
def agregar_evaluacion():
    ...
    try:
        ...
        datos = evaluacion_request_schema.load(
            request.get_json()
        )

        nueva = crear_evaluacion(
            datos
        )
...
def actualizar_evaluacion(id_evaluacion):
    ...
    try:
        ...
        datos = modificar_evaluacion_schema.load(
            request.get_json()
        )

        evaluacion = modificar_evaluacion(
            id_evaluacion,
            datos
        )
        ...
        return success_response(
            data=resultado,
            message=f"Evaluación {id_evaluacion} actualizada."
        )
...
def eliminar_evaluacion_controller(id_evaluacion):
    ...
    try:
        ...
        return success_response(
            message=f"Evaluación {id_evaluacion} eliminada."
        )
